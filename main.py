...
from numpy import random, empty, array
...
with torch.no_grad():
    ...
    if str(source) == '0':
        ...
        video = LoadWebcam(img_size=img_size_yolo)
    else:
        video = LoadImages(source, img_size=img_size_yolo)
    ...
    for path, img, im0s, vid_cap in video:  # And print (tot_frame\current_frame) while call the __next__method
        ...
        p, s, im0 = Path(path), '', array(im0s, copy=True)
        ...
        if len(detections):
            ...
            tracks = mot_tracker.update(np_det[:, 0:-1])
            ...
            tracks = mot_tracker.update(empty((0, 5)))

        # Write tracks on im0
        for x1, y1, x2, y2, id_track in reversed(tracks):
            label = f'{names[0]} n. {int(id_track)}'
            plot_one_box((x1, y1, x2, y2), im0, label=label, color=colors[0], line_thickness=3)

        print(f'{s}Done. ({(t2 - t1):.3f}s) nt({len(tracks)})')

        # Stream results
        if debug:
            # Write yolo detections on im0_yolo
            im0_yolo = array(im0s, copy=True)
            im0_all_tracks = array(im0s, copy=True)
            for *xyxy, conf, cls in reversed(detections):
                label = '%s %.2f' % (names[int(cls)], conf)
                plot_one_box(xyxy, im0_yolo, label=label, color=colors[int(cls)], line_thickness=3)

            # Write all tracks from SORT tracker on img0_all_tracks
            for t in mot_tracker.trackers:
                state = t.get_state()[0]
                id_track = t.id
                plot_one_box(state, im0_all_tracks, label=str(id_track), color=colors[int(cls)], line_thickness=3)

            # Stream  debug results img
            im0_yolo = cv2.resize(im0_yolo, (0, 0), fx=proportion_vid, fy=proportion_vid)
            im0_all_tracks = cv2.resize(im0_all_tracks, (0, 0), fx=proportion_vid, fy=proportion_vid)
            cv2.imshow('all tracks', im0_all_tracks)
            cv2.imshow('yolo detection', im0_yolo)

        # Stream out-put results img
        im0 = cv2.resize(im0, (0, 0), fx=proportion_vid, fy=proportion_vid)
